...
## ***I. Audio Demo***
This section presents audio samples from the PACESpeech dataset alongside speech synthesized by our proposed baseline model. Our model (PACE-TTS) generates expressive speech by conditioning on three inputs: (1) reference speech, a sample utterance used to capture speaker timbre, (2) persona, which encodes the speaker’s demographic and behavioral attributes, and (3) dialog, which provides the situational conversational context. The reference speech is defined for each unique speaker and may or may not come from the same dialog. For comparison, we also include the corresponding outputs generated by other models (MSenC, StyleTTS2, and IndexTTS2).
